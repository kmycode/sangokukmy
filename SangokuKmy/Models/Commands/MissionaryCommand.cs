  ...
  public class MissionaryCommand : Command
  {
    ...
    public override async Task ExecuteAsync(MainRepository repo, Character character, IEnumerable<CharacterCommandParameter> options, CommandSystemData game)
    {
      ...
      else
      {
        ...
        if (character.Religion != ReligionType.Any && character.Religion != ReligionType.None && country.Religion != character.Religion)
        {
          // あえて国教と違う宗教を設定したい場合もあるので
          // await game.CharacterLogAsync("布教しようとしましたが、国教とあなたの宗教が異なるため布教できません");
          // return;
      ...
      if (religion == ReligionType.None)
      {
        await game.CharacterLogAsync("布教しようとしましたが、国教が無神またはあなた自身が無神論者のため布教できません");
        return;
      }

      if (religion == ReligionType.Any)
      {
        await game.CharacterLogAsync("布教しようとしましたが、国教または武将の宗教が設定されていません");
        return;
      }
