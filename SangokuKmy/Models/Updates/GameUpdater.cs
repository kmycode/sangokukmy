        ...
        if (system.GameDateTime.Year >= Config.UpdateStartYear)
        {
          ...
          // 都市建築物の建設または撤去
          {
            ...
            foreach (var subBuilding in availableSubBuildings)
            {
              ...
              {
                countryWarTargets = allCountries.Where(c => !c.HasOverthrown && c.Id != town.CountryId).Select(c => c.Id);
              }
          ...
          */
          ...
          if (system.RuleSet == GameRuleSet.Wandering && system.GameDateTime.Year == 48 && system.GameDateTime.Month == 1)
          {
            foreach (var country in countryData.Where(c => !c.Country.HasOverthrown))
            {
              foreach (var chara in country.Characters)
              {
                ...
                var count = items.Count(i => i.Type == CharacterItemType.CastleBlueprint);
                if (count > 0)
                {
                  chara.Money -= count * 50_0000;
          ...
          if (!system.IsBattleRoyaleMode && !system.IsWaitingReset &&
            (lastBattleMonth.ToInt() + 12 * 12 * 9 == system.IntGameDateTime ||
            (system.GameDateTime.Year == 348 && system.GameDateTime.Month == 1)))
          {
            await AddMapLogAsync(false, EventType.Event, "黄巾が反乱の時期を伺っています");
          }
          if (!system.IsWaitingReset && (!system.IsBattleRoyaleMode && RandomService.Next(0, 20) == 0 || isKokinForce))
          {
            if ((lastBattleMonth.ToInt() + 12 * 12 * 10 <= system.IntGameDateTime) ||
